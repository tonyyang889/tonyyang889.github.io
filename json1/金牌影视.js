var rule = {
  ...
  //JS文件参考：https://gh.927223.xyz/https://raw.githubusercontent.com/xyq254245/xyqonlinerule/main/XYQTVBox.json //https://www.610pkea.com,https://www.gs4x7nq4.com,https://www.gs4x7nq4.com-https://m.hkybqufgh.com,https://m.sizhengxt.com,https://m.9zhoukj.com,https://m.sizhengxt.com,https://m.jiabaide.cn,https://m.sunnafh.com
  ...
  lazy:`js:
     ...
     let json_data = JSON.parse(request('https://www.gs4x7nq4.com/api/mw-movie/anonymous/v2/video/episode/url?clientType=1&id='+pid+'&nid='+nid,{headers: {
    'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/126.0.0.0 Safari/537.36',
  ...
  一级: `js:
        ...
        function shouldFilter(title) {
          for (var i = 0; i < filterMovies.length; i++) {
            if (title.includes(filterMovies[i])) {
              return true;
            }
          }
          return false;
         ...
         list.forEach((it)=>{
            // 检查是否需要过滤
            if (shouldFilter(it.vodName)) {
              // console.log('过滤影片:', it.vodName);
              return; // 跳过这个影片
            }
            d.push({
              title: it.vodName,
  ...
  二级: `js:
    ...
    let key = CryptoJS.SHA1(CryptoJS.MD5(signkey).toString()).toString();
    let kjson = JSON.parse(request('https://www.gs4x7nq4.com/api/mw-movie/anonymous/video/detail?id='+kid,{headers:{
       'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/127.0.0.0 Safari/537.36',
        'Accept': 'application/json, text/plain, */*',
        'deviceId': '63ffad23-a598-4f96-85d7-7bf5f3e4a0a2',
        'sign': key,
    ...
    let kurls = kjson.episodeList.map(function(it) => {
      return it.name + "$" + "https://www.gs4x7nq4.com/vod/play/"+kid+"/sid/"+it.nid
    }).join('#');
    VOD = {
      vod_id: kid,
